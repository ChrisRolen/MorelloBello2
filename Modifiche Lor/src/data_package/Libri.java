...
public class Libri {
	...
	public static ArrayList<Libri> carrello = new ArrayList<Libri>();
        public static ArrayList<Integer> numerodicopie = new ArrayList<Integer>();
	
	public Libri(String ISBN) throws SQLException{
		...
		this.disponibile=rs.getBoolean("disponibile");
                ...
                rs=stmt.executeQuery("select nome from generi,genere_libro where genere=id and ISBN="+ISBN);
                while(rs.next()){
        ...
        public void set_copie(int copie) throws SQLException{
            String query="update libri set copie_vendute="+copie+" where ISBN='"+ISBN+"'";
            pstmt=conn.prepareStatement(query);
            pstmt.execute();
            numberofsoldcopies=copie;
        }
	...
	public void modify(String ISBN, String titolo, String editore, String anno, String prezzo, String descrizione, String punti, String copie_vendute, String autori, String generi,boolean disponibile) throws SQLException{
            String query="update libri set titolo=?, casa_editrice=?, anno_pubblicazione=? , prezzo=?, descrizione=?, punti_card=?, disponibile=? where ISBN='"+ISBN+"'";
            pstmt=conn.prepareStatement(query);
            pstmt.setString(1, titolo);
            pstmt.setString(2, editore);
            pstmt.setInt(3, Integer.parseInt(anno));
            pstmt.setFloat(4, Float.parseFloat(prezzo));
            pstmt.setString(5, descrizione);
            pstmt.setInt(6, Integer.parseInt(punti));
            pstmt.setBoolean(7, disponibile);
            pstmt.execute();
	...
	public static ArrayList<Libri> classify(String genere, ArrayList<Libri> libris){
		ArrayList<Libri> classifica = new ArrayList<Libri>();
		ArrayList<Libri> trueclassifica = new ArrayList<Libri>();
		
		for(Libri a:libris){
                        if(a.getGenre().contains(genere)){
                            classifica.add(a);
                        }
		}
		...
		classifica.sort(new Comparator<Libri>(){
			...
			public int compare(Libri arg0, Libri arg1) {
                ...
                for(int i = 0; i<5; i++){
                    trueclassifica.add(classifica.get(i));
                }
                
		return trueclassifica;
	}
	...
	public ArrayList<Libri> classify() throws SQLException{
		...
		while(rs.next()){
			Libri book = new Libri(rs.getString("IBSN"));
			classifica.add(book);
		}
	...
	public static void  new_book(Utenti utente,String title, String author, String editor,int year, String ISBN,String genre,float price, String description, int numberofpoints) throws SQLException{
		//Libri.carrello;
		
		if(utente.isAdmin()){
                    Connection conn =database.connection();
		String query = " insert into libri (titolo, casa_editrice, anno_pubblicazione, ISBN, prezzo, descrizione, punti_card)"
			        + " values (?, ?, ?, ?, ?, ?, ?)";
    ...
    public static void add_author(String ISBN,int codice,Connection conn) throws SQLException{
        ...
        ResultSet rs=stmt.executeQuery("select * from autori_libri where ISBN='"+ISBN+"' and codice="+codice);
        if(!rs.next()){
        ...
        public static void add_genre(String ISBN,int codice,Connection conn) throws SQLException{
            Statement stmt=conn.createStatement();
            ResultSet rs=stmt.executeQuery("select * from genere_libro where ISBN='"+ISBN+"' and genere="+codice);
            if(!rs.next()){
